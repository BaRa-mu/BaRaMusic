...
def render_tab2():
    ...
    with st.sidebar:
        ...
        # 세션 스테이트 값을 value로 받아 즉시 출력
        k_title = st.text_input("한글 제목", value=st.session_state.get('k_sync', ""), key="k_title_field")
        e_title = st.text_input("영어 제목", value=st.session_state.get('e_sync', ""), key="e_title_field")
    ...
    if not st.session_state.get('img_gen_done') and not gen_btn:
        ...
        def render_preview_block(label, key_id, ratio_class):
            st.markdown(f"### {label}")
            with st.container():
                c1, c2, c3, c4 = st.columns(4)
                with c1:
                    # [핵심] 한글은 한글명, 영어는 영어명 (스타일 미리보기 포함)
            ...
            eff_class = "glow-w" if "글로우" in eff_sel else "glow-n" if "네온" in eff_sel else "sh-d"
        ...
        # 규격별 출력 (쇼츠 9:16 준수)
        render_preview_block("유튜브 메인 (16:9)", "yt", "dim-16-9")
        render_preview_block("틱톡 (9:16)", "tk", "dim-9-16")
        if shorts_count > 0:
            for i in range(shorts_count):
                render_preview_block(f"쇼츠 #{i+1} (9:16)", f"sh_{i}", "dim-9-16")
