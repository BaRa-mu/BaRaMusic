...
def render_tab2():
    ...
    with st.sidebar:
        ...
        audio_file = st.file_uploader("Browse files", type=["mp3", "wav", "m4a"], key="img_audio_up_main", label_visibility="collapsed")
        
        # [핵심] 파일 업로드 시 즉시 파싱하여 세션 스테이트 업데이트
        if audio_file:
            raw_name = os.path.splitext(audio_file.name)[0]
            if st.session_state.get('last_f_id') != audio_file.name:
                if "_" in raw_name:
                    k_p, e_p = raw_name.split("_", 1)
                    st.session_state['k_sync_val'] = k_p
                    st.session_state['e_sync_val'] = e_p
                else:
                    st.session_state['k_sync_val'] = raw_name
                    st.session_state['e_sync_val'] = raw_name
                st.session_state['last_f_id'] = audio_file.name
                st.rerun() # 파싱값을 텍스트 박스에 즉시 뿌리기 위해 재실행

        st.divider()
        st.subheader("🏷️ 이미지 제목 (자동 파싱)")
        k_title = st.text_input("한글 제목", value=st.session_state.get('k_sync_val', ""), key="k_title_final")
        e_title = st.text_input("영어 제목", value=st.session_state.get('e_sync_val', ""), key="e_title_final")
    ...
    # --- [3. CSS: 회색 배경, 520px 고정, 드롭다운 폰트 실시간 스타일] ---
    ...
    st.markdown(f"""
        ...
        /* [해결] 드롭다운(selectbox) 내 폰트 스타일 실시간 미리보기 */
