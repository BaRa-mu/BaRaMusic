...
def render_tab2():
    ...
    if 'k_sync' not in st.session_state:
        st.session_state['k_sync'] = ""
    if 'e_sync' not in st.session_state:
        st.session_state['e_sync'] = ""
    if 'img_gen_done' not in st.session_state:
        st.session_state['img_gen_done'] = False

    # [3. 핵심 파싱 로직: 콜백 함수]
    def sync_titles():
        uploaded_file = st.session_state.get('audio_up')
        if uploaded_file is not None:
            raw_fname = os.path.splitext(uploaded_file.name)[0]
            if "_" in raw_fname:
                k_p, e_p = raw_fname.split("_", 1)
                st.session_state['k_sync'] = k_p
                st.session_state['e_sync'] = e_p
            else:
                st.session_state['k_sync'] = raw_fname
                st.session_state['e_sync'] = raw_fname

    # [4. 사이드바 구성]
    with st.sidebar:
        st.divider()
        st.subheader("📂 음원 파일 업로드")
        st.file_uploader("Browse files", type=["mp3", "wav", "m4a"], key="audio_up", 
                         on_change=sync_titles, label_visibility="collapsed")

        st.divider()
        st.subheader("🏷️ 이미지 제목 (자동 파싱)")
        k_title = st.text_input("한글 제목", value=st.session_state['k_sync'], key="k_input")
        e_title = st.text_input("영어 제목", value=st.session_state['e_sync'], key="e_input")
        ...
        shorts_count = st.number_input("쇼츠 이미지 개수 (0~5)", 0, 5, 0, step=1, key="shorts_num")
        
        st.divider()
        gen_btn = st.button("🚀 이미지 생성 시작", type="primary", use_container_width=True)

    # --- [5. CSS: 회색 배경, 520px 고정, 드롭다운 폰트 스타일] ---
    ...
    st.markdown(f"""
        ...
        .preview-box:hover .dl-btn {{ opacity: 1; }}
        </style>
    """, unsafe_allow_html=True)

    # --- [6. 화면 출력 영역] ---
    if not st.session_state['img_gen_done'] and not gen_btn:
        # [생성 전] 회색 화면 및 중앙 제목
        st.markdown(f"""
        ...
        if gen_btn:
            with st.spinner("이미지 생성 중..."): time.sleep(1.2)
