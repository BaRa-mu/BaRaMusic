...
def render_tab2():
    ...
    with st.sidebar:
        ...
        if audio_file:
            ...
            if st.session_state.get('last_seen_f') != audio_file.name:
                if "_" in fname:
                    kp, ep = fname.split("_", 1)
                    st.session_state['ksync'] = kp
                    st.session_state['esync'] = ep
                else:
                    st.session_state['ksync'] = fname
                    st.session_state['esync'] = fname
                st.session_state['last_seen_f'] = audio_file.name
                st.rerun()

        st.divider()
        st.subheader("🏷️ 이미지 제목 (파싱)")
        k_title = st.text_input("한글 제목", value=st.session_state.get('ksync', ""), key="k_title_input")
        e_title = st.text_input("영어 제목", value=st.session_state.get('esync', ""), key="e_title_input")
        ...
        # [변경] 슬라이더를 숫자 입력으로 교체
        shorts_count = st.number_input("쇼츠 이미지 개수 (0~5)", min_value=0, max_value=5, value=0, step=1, key="shorts_num_input")
    ...
    f_imports = "".join([f"@import url('https://fonts.googleapis.com/css2?family={f['family'].replace(' ', '+')}&display=swap');" for f in all_fonts])
        ...
        st.markdown(f"""
            <div class="preview-canvas dim-16-9">
                <div class="title-eff">
                    ...
                    <div style="font-family: 'Dancing Script', cursive; font-size: 28px; margin-top: 20px;">{e_title if e_title else 'English Title'}</div>
                </div>
            </div>
        """, unsafe_allow_html=True)
    else:
