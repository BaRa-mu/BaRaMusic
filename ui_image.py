...
def render_tab1():
    # [수정] 기존 대비 약 20% 여백을 확장한 중립적 CSS
    st.markdown("""
        <style>
        /* 위젯 간 수직 간격 12px로 확대 (기존 10px 대비 20% 증가) */
        [data-testid="stVerticalBlock"] > div { margin-top: 2px !important; margin-bottom: 12px !important; }
        
        /* 입력창 높이 35px로 조정하여 조작 편의성 증대 */
        div[data-baseweb="select"] > div, .stTextInput input {
            height: 35px !important; min-height: 35px !important;
            padding: 0px 12px !important; display: flex !important; align-items: center !important; font-size: 14px !important;
        }
        
        /* 라벨 여백 상향 조정 (7px) */
        .stSelectbox label, .stRadio label, .stTextInput label {
            font-size: 13px !important; font-weight: 600 !important; color: #444 !important;
            margin-bottom: 5px !important; padding-top: 7px !important;
        }
    ...
    with r_col:
        ...
        if gen_btn:
            ...
            with st.spinner("Gemini 3.1 Pro가 대곡 작사 중..."):
                try:
                    ...
                    # 제목 분리 파싱
                    title_full = res.split("[TITLE]")[1].split("[PROMPT]")[0].strip()
                    parts = title_full.split('_')
                    st.session_state.gen_title_kr = parts[0]
                    st.session_state.gen_title_en = parts[1] if len(parts) > 1 else "Song"
        ...
        # [수정] 결과물 출력 시 제목 한 줄 배치 적용
        if 'gen_title_kr' in st.session_state:
            with st.container(border=True):
                st.write("**1. 🎵 Title**")
                t1, t2 = st.columns(2)
                with t1: st.code(st.session_state.gen_title_kr)
                with t2: st.code(st.session_state.gen_title_en)
