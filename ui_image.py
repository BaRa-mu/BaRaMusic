...
def render_tab2():
    ...
    # 제목 효과 15개 이상
    EFFECT_LIST = ["화이트 글로우", "네온 핑크 광채", "시네마틱 블랙 쉐도우", "골든 샤인", "스모키 안개", "3D 입체 돌출", "크롬 메탈릭", "타오르는 불꽃", "차가운 얼음", "글리치 노이즈", "투명 유리", "은은한 외부 광채", "진한 드롭 쉐도우", "무지개 그라데이션", "고딕 다크 글로우"]
    ...
    with st.sidebar:
        ...
        # [해결] 파일 업로드 즉시 파싱하여 입력창 동기화
        if audio_file:
            current_fn = os.path.splitext(audio_file.name)[0]
            if st.session_state.get('last_fn_check') != audio_file.name:
                if "_" in current_fn:
                    k_res, e_res = current_fn.split("_", 1)
                    st.session_state['k_t_sync'] = k_res
                    st.session_state['e_t_sync'] = e_res
                else:
                    st.session_state['k_t_sync'] = current_fn
                    st.session_state['e_t_sync'] = current_fn
                st.session_state['last_fn_check'] = audio_file.name

        st.divider()
        st.subheader("🏷️ 이미지 제목 (입력)")
        k_title = st.text_input("한글 제목", value=st.session_state.get('k_t_sync', ""), key="k_title_input_field")
        e_title = st.text_input("영어 제목", value=st.session_state.get('e_t_sync', ""), key="e_title_input_field")
    ...
    # --- [3. CSS: 회색 배경, 520px 제한, 폰트 스타일 미리보기] ---
    ...
    st.markdown(f"""
        ...
        /* [해결] 드롭다운 내 폰트 스타일 미리보기 적용 */
        div[data-baseweb="select"] li {{ font-size: 1.1rem !important; padding: 8px !important; }}
        /* 한글 폰트 스타일 */
        { "".join([f'li[id*="option-{i}"] {{ font-family: "{K_FONTS[i]["family"]}", cursive !important; }}' for i in range(len(K_FONTS))]) }
        /* 영어 폰트 스타일 (한글 리스트 뒤에 위치) */
        ...
        .title-eff {{
            ...
            text-shadow: 0 0 15px rgba(255,255,255,1), 0 0 30px rgba(255,255,255,0.6), 2px 2px 5px rgba(0,0,0,1);
        }}
    ...
    if not st.session_state.get('img_gen_done') and not gen_btn:
        # [상태: 생성 전] 회색 화면 및 제목 중앙 배치
        st.markdown(f"""
            <div class="preview-canvas dim-16-9">
                <div class="title-eff">
                    <div style="font-family: 'Aggro', cursive; font-size: 45px;">{k_title if k_title else '한글제목'}</div>
                    <div style="font-family: 'Dancing Script', cursive; font-size: 28px; margin-top: 15px;">{e_title if e_title else 'English Title'}</div>
                </div>
            </div>
        """, unsafe_allow_html=True)
    else:
