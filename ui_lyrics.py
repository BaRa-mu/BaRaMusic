...
SESSION_MAP = {
    "신디사이저": "아르페지에이터 시퀀스, 딥 베이스 신스, 공간감 있는 앰비언트 패드, 리버브 드럼",
...
def render_tab1():
    ...
    with st.sidebar:
        ...
        genre = st.selectbox(f"🎸 {target} 음악 장르", CCM_GENRES if target == "CCM" else POP_GENRES, key="genre_list_sel")
        song_atm = st.selectbox("🌈 곡 분위기 상세", ["밝고 희망찬", "차분하고 정적인", "웅장하고 힘있는", "슬프고 서정적인", "몽환적인", "긴박한", "따뜻한", "거친", "우아한", "애절한", "담백한", "세련된", "목가적인", "장엄한", "에너제틱한"], key="song_atm_sel")
        ...
        if st.button("🚀 AI 가사 및 세션 구성 시작", type="primary", use_container_width=True):
            ...
            # [구조적 해결] 시간 채우기 금지 프롬프트 조립
            neg_prompt = "NO looping, NO repeating sections, NO 2nd song start, NO padding instrumental time." if strict_end else ""
            
            # [구조적 해결] 가사 마지막에 강제 종료 태그 삽입
            ending_tags = "\n\n[Outro]\n(Natural fade out to silence)\n[END]\n[Hard Stop]\n[Silence]"
            
            st.session_state.ai_session = SESSION_MAP.get(main_inst, "기본 구성")
            st.session_state.final_lyrics = f"[Verse 1]\n{subject}의 시작...\n\n(생성된 가사 본문...)" + ending_tags
            st.session_state.negative_p = neg_prompt

    # 결과 출력부
    st.title("🎼 AI 음악 제작 분석")
    if st.session_state.get('music_ready'):
        with st.container(border=True):
            st.subheader(f"🤖 AI 추천 세션 (메인: {main_inst})")
            st.info(f"👉 **{st.session_state.ai_session}**")
            st.warning(f"⚠️ 적용된 제약: {st.session_state.negative_p}")

        st.divider()
        st.subheader("📝 생성된 가사 (종료 신호 포함)")
        st.text_area("가사 확인 (마지막 [END] 태그를 확인하세요)", value=st.session_state.final_lyrics, height=400)
