...
def render_tab1():
    # 사이드바 하단에 기존 드롭다운 메뉴들 복구
    with st.sidebar:
        st.subheader("🎵 음악 생성 설정")
        subject = st.text_input("💡 곡 주제", placeholder="곡의 핵심 주제를 입력하세요", key="l_subject")
...
(반복)""" + ending_tags
            ...
            st.session_state.res_prompt = f"Professional high-fidelity {genre} for {target}. Mood: {lyric_mood}, {song_atm}. Tempo: {tempo}. Extended 3-8 minute flow. Lead {v_type} vocal ({vocal_style}) with 24-bit clarity. Instrumental: {main_inst} and {s_info}. Engineering: warm saturation, silky highs, no digital clipping. Hard stop after final lyric. High-end soundstage for '{subject}'."
        ...
        st.divider()
        # 가사 수정 칸 (표준 높이)
        st.subheader("📝 생성 가사 (직접 수정 및 편집 가능)")
        ...
        st.text_area("프롬프트 확인", value=st.session_state.res_prompt, height=250, key="prompt_final_view")
