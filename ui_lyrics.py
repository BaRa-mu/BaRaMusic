...
def render_tab1():
    # --- 드롭다운 박스 자체의 크기를 줄이는 초정밀 CSS ---
    st.markdown("""
        <style>
        /* 1. 드롭다운 박스(Selectbox) 전체 높이 및 배경 축소 */
        div[data-baseweb="select"] {
            min-height: 28px !important;
            height: 28px !important;
        }
        
        /* 2. 드롭다운 내부의 패딩과 폰트 정렬 조정 */
        ...
        /* 5. 위젯 간의 불필요한 수직 간격 제거 */
        [data-testid="stVerticalBlock"] > div {
            margin-top: -15px !important;
        }

        /* 6. 라디오 버튼(보컬 등) 높이 및 간격 조절 */
    ...
    with left_col:
        ...
        if gen_btn:
            ...
            with st.spinner("가사와 제목을 완벽히 매칭하는 중..."):
                time.sleep(0.5)
                # 제목 로직: 주제의 핵심 키워드를 반영하여 생성
                if any(x in subject for x in ["기쁨", "즐겁", "행복"]):
                    kr_t, en_t = "참된 기쁨의 노래", "Song of True Joy"
                ...
                p_base = f"Professional {target} track. Genre: {s_genre}. Mood: {s_mood}. Tempo: {s_tempo}. Vocals: {s_vocal}. Main Inst: {s_inst}. Style: {s_lyric_style}. Title: {kr_t}. "
                while len(p_base) < 880: p_base += "Mastered for high-fidelity audio, wide stereo field, and cinematic emotional depth. "
                st.session_state.gen_prompt = p_base[:995]
                st.session_state.gen_lyrics = f"[Intro]\n({s_mood} 분위기를 여는 {s_inst} 전주)\n\n[Verse 1]\n{subject}의 마음을 담아 노래합니다\n[Chorus]\n영원한 평안 속에 나 노래하리\n[Outro]\n({s_inst}의 잔잔한 여운으로 마무리)"
