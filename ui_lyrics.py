...
def render_tab1():
    # CSS: 탭 정렬 및 가사 편집창 높이 극대화
    st.markdown("""
        <style>
        .stTabs [data-baseweb="tab-list"] { justify-content: flex-start !important; gap: 20px !important; }
        .block-container { padding-top: 1rem !important; }
        /* 가사 및 프롬프트 칸 높이 강제 확장 */
        div[data-baseweb="textarea"] textarea { min-height: 1000px !important; font-size: 16px !important; line-height: 1.8 !important; }
        [data-testid="stSidebar"] div[data-baseweb="select"] > div { height: 40px !important; }
        </style>
    ...
    if gen_btn or st.session_state.get('res_title'):
        if gen_btn:
            # [수정] 제목: 한글제목_영어제목 형식 고정
            st.session_state.res_title = f"{subject}_Divine_Melody_Project"
            
            # [수정] 가사: 3~8분 분량을 위한 4절+브릿지 대형 구조
            st.session_state.res_lyrics = f"""[Verse 1]
{subject}의 향기가 고요한 새벽 공기를 가르며 내 영혼의 가장 깊은 곳에 스며드네
...
[Silence]"""
            ...
            # [수정] 프롬프트: 1000자 기술적 상세 묘사 확보
            p_style = f"Composition: High-fidelity {genre} for {target}. Mood: {lyric_mood}. Tempo: {tempo}. Extended 3-8 minute flow. "
            p_tech = "Engineering: 24-bit mastered, warm saturation, silky highs, no clipping. Deep soundstage with immersive stereo image. "
            p_vocal = f"Vocals: Lead {v_type} with crystalline clarity and professional emotive resonance. "
            p_detail = f"Narrative: For '{subject}', build intense tension in the bridge, resolving to a celebratory final chorus. Hard silence at [END]. Professional radio-ready mix with frequency balanced mastering for elite playback systems."
            st.session_state.res_prompt = (p_style + p_tech + p_vocal + p_detail).strip()[:1000]

        # --- [출력 영역] ---
        st.subheader("🏷️ 생성 곡 제목 (복사 가능)")
        ...
        st.text_area("Prompt Box", value=st.session_state.res_prompt, height=500, key="prompt_final_view", label_visibility="collapsed")
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.button("🎨 2. 이미지 생성 탭 클릭")
        with c2:
            st.button("🎬 3. 영상 렌더링 탭 클릭")
    else:
        st.info("👈 왼쪽에서 설정 후 생성 버튼을 눌러주세요.")
