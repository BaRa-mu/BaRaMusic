...
VOCALS = {
    "남성": ["허스키하며 애절한 저음 남성", "맑고 투명한 미성의 고음 남성"],
    "여성": ["청아하고 이슬 같은 맑은 고음 여성", "몽환적이고 신비로운 분위기의 음색 여성"],
    "듀엣": ["서로 다른 음색의 애틋한 남녀 듀엣", "완벽한 하모니의 동성 2인조 보컬"]
}
INSTRUMENTS = ["신디사이저", "그랜드 피아노", "어쿠스틱 기타", "일렉 기타", "첼로"]
SESSION_MAP = {"신디사이저": "아르페지에이터, 딥 베이스 신스", "그랜드 피아노": "바이올린 섹션, 콘트라베이스", "어쿠스틱 기타": "카혼, 젬베", "일렉 기타": "드럼 세트, 락 베이스", "첼로": "피아노 반주, 소프라노 스트링"}

def render_tab1():
    # CSS: 탭 왼쪽 정렬 및 텍스트 영역 강제 확장
    st.markdown("""
        <style>
...
{subject}의 놀라운 축복이 마르지 않는 강물처럼 우리 삶에 넘쳐하도록 흐르리
...
(반복)""" + ending_tags
            ...
            # [프롬프트] 700~1000자 기술적 묘사 보강
            s_info = SESSION_MAP.get(main_inst, "")
            p_head = f"A professional high-fidelity {genre} production for {target}. Mood: {lyric_mood}, Atmosphere: {song_atm}. Tempo: {tempo}. "
            p_vocal = f"Vocals: Feature a {vocal_style} lead {v_type} performance with crystalline clarity and deep emotional resonance. Harmonies must be multi-layered and professional. "
            p_inst = f"Instrumentation: Driven by {main_inst} and supported by {s_info}. Wide immersive stereo soundstage, frequency-balanced mastering. "
            p_tech = "Engineering: 24-bit mastered audio, warm analog saturation, silky highs, no digital clipping. Explicit dynamic range from intimate verses to celebratory choruses. "
            p_end = f"Constraint: { 'CRITICAL: NO looping. MUST end in absolute silence after [END] tag.' if strict_end else 'Standard end.' } "
            p_detail = f"Narrative: Track for '{subject}' requires a 3-8 minute cinematic journey. Build tension during the bridge and resolve into a grand celebratory finale. Meticulous care in mastering ensures no artificial loops or time padding."
            st.session_state.res_prompt = (p_head + p_vocal + p_inst + p_tech + p_end + p_detail).strip()[:1000]
