...
def render_tab2():
    ...
    # [2. 핵심 파싱 로직: 콜백 함수]
    def sync_titles():
        if st.session_state.audio_up:
            raw_fname = os.path.splitext(st.session_state.audio_up.name)[0]
            if "_" in raw_fname:
    ...
    with st.sidebar:
        ...
        k_title = st.text_input("한글 제목", value=st.session_state.get('k_sync', ""), key="k_input")
        e_title = st.text_input("영어 제목", value=st.session_state.get('e_sync', ""), key="e_input")
    ...
    # --- [4. CSS: 회색 배경, 520px 고정, 드롭다운 폰트 스타일] ---
