...
def render_tab3():
    ...
    with st.sidebar:
        ...
        vid_subject = st.text_input("💡 영상 주제", placeholder="영상 연출 주제 입력", key="v_subject")
    ...
    if gen_vid_btn or st.session_state.get('vid_ready'):
        ...
        # [결과 고정] 제목: 한글_영어제목 형식
        st.session_state.res_vid_title = f"{vid_subject}_Cinematic_Motion_Vision"
        ...
        st.session_state.res_vid_prompt = (
            ...
            f"Hyper-realistic 4k, fluid motion, professional color grading, visual effects, high-end production quality."
        )

        st.subheader("🏷️ 영상 제목 (한글_영어제목)")
        ...
        st.info("👈 왼쪽 사이드바에서 영상 설정을 마치고 생성 버튼을 눌러주세요.")
