...
def render_tab3():
    ...
    with col1:
        gen_m = st.checkbox("📺 가로 메인")
        img_m = st.file_uploader("메인 이미지 (16:9)", type=['png','jpg']) if gen_m else None
    with col2:
        gen_t = st.checkbox("📱 세로 틱톡")
        img_t = st.file_uploader("틱톡 이미지 (9:16)", type=['png','jpg']) if gen_t else None
