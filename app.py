...
st.set_page_config(page_title="AI 뮤직비디오 팩토리 (수동 업로드 전용)", page_icon="🎵", layout="wide")
...
st.write("제목이 포함된 이미지를 직접 업로드하세요. 영상 렌더링 후 앱에서 바로 유튜브로 업로드할 수 있습니다.")
...
st.sidebar.divider()
lyrics = st.sidebar.text_area("📝 가사 입력 (비워두면 스크롤 없음, []는 자동삭제)", height=200)
...
if st.button("🚀 비디오 렌더링 시작", use_container_width=True):
    ...
    if uploaded_audio is None:
        ...
        st.error("⚠️ 메인 영상 배경 이미지를 업로드해주세요.")
        ...
        try:
            ...
            step_title.markdown("#### ✨ 렌더링 완료! 아래에서 미리보기 및 유튜브 업로드를 진행하세요.")
...
if st.session_state.is_completed:
    ...
    if not os.path.exists(st.session_state.main_video_path):
        st.error("🚨 서버 메모리 부족으로 파일이 유실되었습니다. 쇼츠 개수를 줄여서 다시 시도해주세요.")
        st.session_state.is_completed = False
    else:
